function formatFileSize(bytes) {
  const sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB'];
  if (bytes === 0) return '0 Bytes';
  const i = Math.floor(Math.log(bytes) / Math.log(1024));
  const fileSize = bytes / Math.pow(1024, i);
  return `${fileSize.toFixed(2)} ${sizes[i]}`;
}



var FileUploadWithPreview = function (I) {
  "use strict"; const b = ""; var e = (i => (i.IMAGE_ADDED = "fileUploadWithPreview:imagesAdded", i.IMAGE_DELETED = "fileUploadWithPreview:imageDeleted", i.CLEAR_BUTTON_CLICKED = "fileUploadWithPreview:clearButtonClicked", i.IMAGE_MULTI_ITEM_CLICKED = "fileUploadWithPreview:imageMultiItemClicked", i))(e || {}); 
  const t = ":upload:", 
  n = "data:image/png;base64,", 
  l = "data:image/png;base64,", 
  r = "data:image/png;base64,", 
  c = "data:image/png;base64,", 
  d = "data:image/png;base64,", 
  S = "multi-item-clear-animation", 
  u = "Choose file...", 
  ww = "filename", 
  m = "Browse", 
  h = "files selected", 
  s = "Upload", 
  k = () => Math.random().toString(16).slice(2); class p {
    constructor(A, g = {}) {
      if (this.options = { 
        accept: "*", 
        images: { 
          backgroundImage: d, 
          baseImage: n, 
          successFileAltImage: c, 
          successPdfImage: l, 
          successVideoImage: r 
        }, 
        maxFileCount: 0, 
        multiple: !1, 
        presetFiles: [], 
        showDeleteButtonOnImages: !0, 
        text: { 
          browse: m, 
          filename: ww, 
          chooseFile: u, 
          label: s, 
          selectedCount: h 
        }
      }, !A) throw new Error("No uploadId found. You must initialize file-upload-with-preview with a unique uploadId."); 
      this.uploadId = A, 
      this.cachedFileArray = []; 
      const { 
        accept: E, 
        maxFileCount: C, 
        multiple: B, 
        presetFiles: Q, 
        showDeleteButtonOnImages: a 
      } = g; 
      this.options.showDeleteButtonOnImages = a ?? !0, 
      this.options.maxFileCount = C ?? 0, 
      this.options.presetFiles = Q ?? [], 
      this.options.multiple = B ?? !1, 
      this.options.accept = E ?? 
      this.options.accept; const { 
        browse: o,
        filename: wwxy,
         chooseFile: f, 
         label: y, 
         selectedCount: D 
        } = g.text || {}; 
        this.options.text.browse = o ?? this.options.text.browse, 
        this.options.text.filename = wwxy ?? this.options.text.filename, 
        this.options.text.label = y ?? s, 
        this.options.text.selectedCount = D ?? this.options.text.selectedCount; 
        const J = document.querySelector(`.custom-file-container[data-upload-id="${this.uploadId}"]`); 
        if (!J) throw new Error(`Could not find a 'custom-file-container' with the id of: ${this.uploadId}`); 
      this.el = J, 
      this.el.innerHTML += `
      <a class="clear-button" href="javascript:void(0)" title="Clear Image">&times;</a>; 
      <label class="js_selecor btcd-f-wrp bg-primary bg-opacity-10 p-2 text-primary w-100">
        <span class="input-visible">
        <button class="btcd-inpBtn btn btn-primary mb-0 me-2" type="button">
            <i class="bi bi-paperclip me-1"></i> ${this.options.text.browse}
          </button>
          <div class="browse-text">${f ?? this.options.text.chooseFile}</div>
        </span>
        <input accept="${this.options.accept}" name="${this.options.text.filename}${this.options.multiple ? "[]" : ""}" aria-label="Choose File" class="input-hidden" id="file-upload-with-preview-${A}"
          ${this.options.multiple ? "multiple" : ""} type="file" />  
      </label>
      <div class="image-preview">

      </div>
    `;

    const R = this.el.querySelector(".custom-file-container .input-hidden"), 
    F = this.el.querySelector(".custom-file-container .input-visible"), 
    XX = this.el.querySelector(".custom-file-container .js_selecor"),
    xtext = this.el.querySelector(".custom-file-container .browse-text"),
    w = this.el.querySelector(".custom-file-container .image-preview"), 
    v = this.el.querySelector(".custom-file-container .clear-button"); 
    if (R != null && F != null && w != null && v != null) this.inputHidden = R, 
    this.inputVisible = F, 
    this.ffselector = XX, 
    this.fftextt = xtext, 
    this.imagePreview = w, 
    this.clearButton = v; 
    else throw new Error(`Cannot find all necessary elements for the id: ${this.uploadId}`); 
    const { 
      backgroundImage: M, 
      baseImage: U, 
      successFileAltImage: L, 
      successPdfImage: P, 
      successVideoImage: G 
    } = g.images || {}; 
    this.options.images.baseImage = U ?? this.options.images.baseImage, 
    this.options.images.successPdfImage = P ?? this.options.images.successPdfImage, 
    this.options.images.successVideoImage = G ?? this.options.images.successVideoImage, 
    this.options.images.successFileAltImage = L ?? this.options.images.successFileAltImage, 
    this.options.images.backgroundImage = M ?? this.options.images.backgroundImage, 
    this.addImagesFromPath(this.options.presetFiles), 
    this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, 
    this.bindClickEvents()
    } 
    bindClickEvents() { 
      this.inputHidden.addEventListener("change", A => {
         const g = A.target, { files: E } = g; E != null && (this.addFiles(E), g.value = "") 
      }, !0), 
      this.clearButton.addEventListener("click", () => { 
        const A = { detail: { uploadId: this.uploadId } }, g = new CustomEvent(e.CLEAR_BUTTON_CLICKED, A); window.dispatchEvent(g), this.resetPreviewPanel() 
      }, !0), 
      this.imagePreview.addEventListener("click", A => {
        const g = A.target; if (g) { 
          if (g.matches(".custom-file-container .image-preview-item-clear-icon")) { 
            const E = g.getAttribute("data-upload-name"), 
            C = this.cachedFileArray.findIndex(({ name: B }) => B === E);
             this.deleteFileAtIndex(C) 
            } 
          if (g.matches(".custom-file-container .image-preview-item")) { 
            const E = g.querySelector(".image-preview-item-clear-icon"), 
            C = E == null ? void 0 : E.getAttribute("data-upload-name"), 
            B = this.cachedFileArray.findIndex(({ name: o }) => o === C); 
            if (B < 0) return; const Q = { 
              detail: { 
                cachedFileArray: this.cachedFileArray, 
                file: this.cachedFileArray[B], 
                index: B, uploadId: this.uploadId } 
            }, a = new CustomEvent(e.IMAGE_MULTI_ITEM_CLICKED, Q); window.dispatchEvent(a) 
          } 
        } 
      }) 
    } 
    async addImagesFromPath(A) { 
      A.forEach(async g => { try { const E = "image/jpeg", B = await (await fetch(g, { mode: "cors" })).blob(), Q = new File([B], "preset-file", { type: B.type || E }); this.addFiles([Q]) } catch (E) { E instanceof Error && console.warn(`${E.message.toString()}`), console.warn("Image cannot be added to the cachedFileArray.") } }) 
    } 
    addFiles(A) { 
      if (!A.length) return; 
      let g = Array.from(A);
       if (this.options.multiple && this.options.maxFileCount > 0) { 
        const Q = this.cachedFileArray.length + g.length - this.options.maxFileCount; 
        Q > 0 && (g = g.slice(0, g.length - Q)) 
      } 
      this.options.multiple || (this.cachedFileArray = []),
       g.forEach(B => { const Q = new File([B], `${B.name || "fallback-name"}${t}${k()}`, { type: B.type }); 
       this.cachedFileArray.push(Q), 
       this.addFileToPreviewPanel(Q) }); 
       
       const E = { detail: { addedFilesCount: g.length, cachedFileArray: this.cachedFileArray, files: A, uploadId: this.uploadId } }, C = new CustomEvent(e.IMAGE_ADDED, E); window.dispatchEvent(C) 
    } 
    addFileToPreviewPanel(A) {
      //const browseTextElement = this.el.querySelector('.browse-text');
      
      this.cachedFileArray.length === 0 ? this.fftextt.innerHTML = this.options.text.chooseFile : this.cachedFileArray.length === 1 ? this.fftextt.textContent = A.name.split(t)[0] : this.fftextt.innerHTML = `${this.cachedFileArray.length} ${
        this.options.text.selectedCount}`; 
      const g = new FileReader; g.readAsDataURL(A), 
      g.onload = () => {
        if (!this.options.multiple) {

          this.ffselector.style.display = 'none';
          this.el.classList.add('singleimg');
          
           let B = this.options.images.successFileAltImage; 
           A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? B = `url("${g.result}")` : A.type.match("application/pdf") ? B = `url("${this.options.images.successPdfImage}")` : A.type.match("video/*") && (B = `url("${this.options.images.successVideoImage}")`), 
           this.imagePreview.style.backgroundImage = B; 

  
           return 
          } 
          this.imagePreview.style.backgroundImage = `url("${this.options.images.backgroundImage}")`; 
          const E = B => ` <button type="button" class="btn-close text-reset image-preview-item-clear image-preview-item-clear-icon"  data-upload-name="${B}" title="Remove This File">  </button> `; 
          let C = this.options.images.successFileAltImage; 
          A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? C = g.result : A.type.match("application/pdf") ? C = this.options.images.successPdfImage : A.type.match("video/*") && (C = this.options.images.successVideoImage), 
          this.imagePreview.innerHTML += ` <div class="image-preview-item bg-info bg-opacity-10 p-2 text-primary w-100 mt-2 d-flex align-items-center justify-content-between" data-upload-name="${A.name}"  >
          <img src="${C}" alt="img" title="${A.name.split(t)[0]}"  data-glightbox="" data-gallery="gallery" data-title="${A.name.split(t)[0]}" data-type="image">
          <div class="details"><span title="${A.name.split(t)[0]}">${A.name.split(t)[0]}</span><br><small>${formatFileSize(A.size)}</small></div>
        
        
          ${this.options.showDeleteButtonOnImages ? E(A.name) : void 0} </div>`

          // Reinitialize glightbox after inserting the new elements
        GLightbox({ selector: '[data-glightbox]' });
        }
    } 
    replaceFiles(A) { 
      if (!A.length) throw new Error("Array must contain at least one file."); 
      this.cachedFileArray = A, 
      this.refreshPreviewPanel() 
    } 
    replaceFileAtIndex(A, g) { 
      if (!this.cachedFileArray[g]) throw new Error(`There is no file at index: ${g}`); 
      this.cachedFileArray[g] = A, 
      this.refreshPreviewPanel() 
    } 
    deleteFileAtIndex(A) { 
      if (!this.cachedFileArray[A]) throw new Error(`There is no file at index ${A}`); 
      this.cachedFileArray = [...this.cachedFileArray.slice(0, A),
         ...this.cachedFileArray.slice(A + 1)], 
      this.refreshPreviewPanel(); 
      const g = { 
        detail: { 
          cachedFileArray: this.cachedFileArray, 
          currentFileCount: this.cachedFileArray.length, 
          index: A, uploadId: this.uploadId 
        } 
      }, 
      E = new CustomEvent(e.IMAGE_DELETED, g); 
      window.dispatchEvent(E) 
    } 
    refreshPreviewPanel() { 
      const g = this.imagePreview.querySelectorAll(".image-preview-item"); 
      Array.from(g).forEach(C => C.classList.add(S)), setTimeout(() => {
         if (this.imagePreview.innerHTML = "", !this.cachedFileArray.length) {
           this.resetPreviewPanel(); return 
          } 
          this.cachedFileArray.forEach(C => this.addFileToPreviewPanel(C)) 
        }, 200) 
      } 

    emulateInputSelection() {
       this.inputHidden.click() 
    } 
    resetPreviewPanel() { 
      this.inputHidden.value = "", 
   
      this.fftextt.innerHTML = this.options.text.chooseFile, 

      this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, 

      this.ffselector.style.display = 'block',
      this.el.classList.remove('singleimg');
      
      
      this.imagePreview.innerHTML = "", 
      this.cachedFileArray = [] 
    }
  } 
  return I.DEFAULT_BACKGROUND_IMAGE = d, I.DEFAULT_BASE_IMAGE = n, I.DEFAULT_BROWSE_TEXT = m, I.DEFAULT_CHOOSE_FILE_TEXT = u, I.DEFAULT_FILES_SELECTED_TEXT = h, I.DEFAULT_LABEL_TEXT = s, I.DEFAULT_SUCCESS_FILE_ALT_IMAGE = c, I.DEFAULT_SUCCESS_PDF_IMAGE = l, I.DEFAULT_SUCCESS_VIDEO_IMAGE = r, I.Events = e, I.FileUploadWithPreview = p, I.MULTI_ITEM_CLEAR_ANIMATION_CLASS = S, I.UNIQUE_ID_IDENTIFIER = t, Object.defineProperty(I, Symbol.toStringTag, { value: "Module" }), I
}({});